...
require_once('handlers/lib/payqueue.php');

class UiIPPxpsmail extends UiIPP {
    ...
    protected function _save_mail_relation($childmail,$child,$parent) {
		...
		//no referer always from root invitation
        $data = "\$".$pname."_listmail['$child'] = '".$childmail."'";//.'<'.$parent."';"; //save referer and email
	...
	public function add_user_printer_form($message=null, $name=null, $users=null, $indir=null) {
	    ...
	    $qdiff = abs($printer_quota - $user_quota);
		$paybutton = null;
		if ($qdiff<=1) 
		    $paybutton = $this->pay();
		elseif ($qdiff<10) 
...
<script type="text/javascript" src="view.js"></script>	
	...
	<div id="form_container">	
		...
		<div class="form_description">
			<h2>Printer Quota</h2>
			<p>If print jobs quota reach the limit, please renew.</p>
		</div>						
		<ul >			   
        <li id="li_1" >
		<div class="left">
			<input id="element_1_1" name="ipq" class="element text medium" value="$printer_quota" type="text" readonly>
			<label for="element_1_1">Printer quota</label>
		</div>		
		<div class="right">
			<input id="element_1_2" name="iuq" class="element text large" value="$user_quota" type="text" readonly>
...
EOF;
	...
	public function form_infoprinter($printername=null, $indir=null) {
		...
		return (self::html_window(null, $ret, 'dropbox.printer'));	
    }	
	...
	//quota mail system, ui function ...
	//get printer limit
	protected function get_printer_limit() {
		...
		if (intval($user_quota) > $printer_quota) {
			...
			if ($qdiff<=1) 
			    return ($this->mail_printer_limit($qdiff));
	...
	protected function pay() {
        ...
        $paybutton = $pay->paybutton('test p', 1.99, 'b.alexiou-facilitator@stereobit.gr');//'balexiou@stereobit.com'); 
        unset($pay);
