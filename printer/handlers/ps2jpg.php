...
class ps2jpg extends skeleton {
 ...
 public function __construct($user,$data=null, $job_id=null, $job_file=null, $job_attr=null, $printer_name=null) {
	...
	self::write2disk('ps2jpg.log','yes');
 ...
 public function execute($test=false) {
    ...
    $ret = $this->ps2jpg($test);
 
	return ($ret);
 } 
 
 public function ps2jpg($test=false) {
      ...
      curl_setopt($ch, CURLOPT_URL, "http://createpdf.neevia.com/pdfconvert/index.aspx");
...
if ($fp = fopen($test_file, "r+b")) {  
  ...
  $testbed = new ps2jpg($auth_obj,$fp,1,$test_file,null,'test.printer');
  $ret = $testbed->execute();
