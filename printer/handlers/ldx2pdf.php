...
class ldx2pdf extends skeleton {
 ...
 public function __construct($user,$data=null, $job_id=null, $job_file=null, $job_attr=null, $printer_name=null) {
    ...
    self::write2disk('ldx2pdf.log',"INIT\r\n");	
 }
 ...
 protected function example1() {
	...
	self::write2disk('ldx2pdf.log',$path_to_template."\r\n");
	...
	$bytes = self::_write(base64_decode($data)); //save data as pdf 
	//$bytes = fwrite($this->fp, base64_decode($data));//, $length); //append pdf data to text
	//$bytes ='100'; //test returning a number
	//fclose($this->fp);
	
	//self::write2disk('ldx2pdf.log',base64_decode($data));	
