...
class dboxsave extends skeleton {
 ...
 public function __construct($user,$data=null, $job_id=null, $job_file=null, $job_attr=null, $printer_name=null) {
	...
	self::write2disk('dboxsave.log',date(DATE_RFC822). PHP_EOL);
 ...
 public function execute($debug_mode=false) {
	...
	$filter = 'dboxsave'; //= this class name
    $userstr = $this->jowner ? '-'.$this->jowner : null;		
	$config_file = $this->admin_path .'/'. $filter.$userstr.'-conf'.'.php';	
	self::write2disk('dboxsave.log',"READ CONFIG ($userID):".$config_file. PHP_EOL);
	
	if (is_readable($config_file)) { //load before post
	    include ($config_file);
		//$dbfolder will be read asis inside the file
	}		
	$myfolder = $dbfolder ? $dbfolder : null;
	$myfext = $dbfext ? $dbfext : null;
	//end loading user config file... 	
    ...
    try {
        ...
        if ($debug_mode) {
		  ...
		  echo "<br>DBOXSAVE FOLDER ($userID):". $myfext ."<br>";
		}  
		else {
          self::write2disk('dboxsave.log',"FILE ($userID):".$this->jf. PHP_EOL);
		  self::write2disk('dboxsave.log',"FOLDER ($userID):".$myfolder. PHP_EOL);
		  self::write2disk('dboxsave.log',"FOLDER ($userID):".$myfext. PHP_EOL);
		}  
	    //return true;
		 
		$new_filename = $myfext ? $this->jattr['job-name'].'.'.$myfext : $this->jattr['job-name']; 
        // Upload the file with an alternative filename
        $put = $this->dropbox->putFile($this->jf,$new_filename,$myfolder,true); //alt name,path,override
        ...
        //create directory
        self::write2disk('dboxsave.log',"CREATE DIR:".$myfolder . PHP_EOL); 		
		$this->create_dropbox_directory($myfolder);  		  
    }	
	
    self::write2disk('dboxsave.log',"PUT:$put". PHP_EOL);
