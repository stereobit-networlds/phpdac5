...
class ticketsave extends skeleton {
 ...
 public function save($test=false) {
    ...
    elseif (substr($data,0,2)=='PK') {//xps ziped file 
		...
		$verify_link = 'http://smart-printers.net/ticketprinter.php?t=verify';
		
	    if (!is_dir($date_path)) //must be created before...
	        @mkdir($date_path, 0755);

		$xpsname = $tdoc_name .'.xps';
		$xpsfile = $date_path .'/'. $xpsname;	
			
        $qrcname = $tdoc_name .'-qrpay.png';
		$qrcfile = $date_path .'/'. $qrcname;

		$qrdoc = new qrfactory($verify_link);
		$qrdoc->QRcreate($qrcfile);	

	    //xps add sign with qr code..
		$xpsdoc = new xpsfactory(true, $qrcfile);
		$xpsdoc->xps_load($xpsfile, $this->jf);		
		if ($xpsdoc->xps_resource_sign('sign-test')) {
                if ($bytes = self::_write(@file_get_contents($xpsfile))) {//$data)) //as is
				
                    //save to dropbox..
                    if ($this->idropbox) { 
			            $this->save2dropbox($xpsfile, $date_dir, $xpsname);		
					}				
                    return $bytes;
				}	
                else
