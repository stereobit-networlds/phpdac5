...
class arttimeresize extends skeleton {
 ...
 public function __construct($user,$data=null, $job_id=null, $job_file=null, $job_attr=null, $printer_name=null) {
    ...
    $this->ftp_server = "arttime.gr";	
    $this->ftp_user_name = "smartsoft";
    $this->ftp_user_pass = "2314523145";	
	$this->ftp_path = 'images/product_images/';
	$this->ftp_path_per_size = array('small/','medium/','large/');
 
    self::write2disk('arttimeresize.log',date(DATE_RFC822)."INIT\r\n");	
 }
						...
						if  ($image->image_type) { //is image
							   ...
							   if ($dim_large = $this->autoresize[2]) {
								 ...
								 if (ftp_put($conn_id, $remote_file, $file, FTP_BINARY)) {
                                    @unlink($file);							
                                    self::write2disk('arttimeresize.log',date(DATE_RFC822)."$file transfered\r\n");									
								 }	
							   }								   
...
if ($fp = fopen($test_file, "r+b")) {  
  ...
  $test_image = new arttimeresize($auth_obj,$fp,1,$test_file,null,'test.printer');
