...
EOF;
        $ret  = $this->html_window("Setup new user", $form, $this->printer_name);	
        return ($ret);	
	}	
...
EOF;
	...
	protected function html_get_printer_jobs_info() {
        ...
        //header line..ONLY FOR TEST accounts	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!	
		if (($user==$this->get_printer_admin()) || ($user=='demo') || ($user=='billy')) {
		...
		if (is_array($jobs)) {
			...
			foreach ($jobs as $jid=>$fileattr) {			   
		       ...
		       //big jobname
		       if (mb_strlen($job_name,'utf-8')>40)
			        $jb_name = mb_substr($job_name,0,40,'utf-8').'...';//'bigname';
			   else
			        $jb_name = $job_name;
               $ret .= self::printline(array($i++,$job_time,$job_size,$jb_name,$proceed_state,$edit),
			                           array('left;5%','left;30%','left;10%','left;45%','left;5%','left;5%'),
                                       0,
			                           "center::100%::0::group_article_body::left::0::0::");									   									   
			}
