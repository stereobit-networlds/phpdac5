...
class UiIPPtaxcloud extends UiIPP {
	...
	public function printer_console($action=null, $noauth=false) {
        ...
        //echo 'CONSOLE', 'printername:',$this->printer_name,'>',$action;		  
		...
		if (($this->printer_name) && ($jid = $_GET['job'])) {
		    ...
		    switch ($action) { 
			   ...
			   case 'proceed' :	$ret .= $this->html_proceed_printer_job($jid, false); break;//this override, true for silent exec		   
	...
	//override..add css for html5-dnd ...and encoding dynamic change
	protected function html_header($encoding=null, $reload=null) {
	
	  if ($this->external_use) 
	    return null;
	  
	  //dynamic encoding on show
	  if ($_GET['action']=='show') //in case of showe..force greek encoding
	    $encoding = 'iso-8859-7';
	  else
      ...
      $ret .= '<title>Tax Cloud Server '.$this->server_version.' | stereobit.networlds</title>
               <link href="taxcloud-printer.css" rel="stylesheet" type="text/css" />	  
            </head>
            <body>';
...
EOF;
	...
	public function form_configprinter($printername=null, $indir=null) {
	    ...
	    if ($this->username!=$this->get_printer_admin()) {
		    ...
		    $ret = self::config_filter_form_taxcalc('taxcalc', $printername, $code, $indir);
			return ($ret);
		}     
...
EOF;
	...
	protected function config_filter_form_taxcalc($filter=null, $printername=null, $code=null, $indir=null) {
		...
		$readonly_at_activation = is_array($this->url_activate) ? '' : 'readonly'; //allow edit 1st time in activate 
		$readonly_at_use = is_array($this->url_activate) ? 'readonly' : ''; //allow edit only after activation 

		$iafm = is_array($this->url_activate) ? trim($this->url_activate[0]) : $iafm; //as readed
		$itaxuser_mail = is_array($this->url_activate) ? trim($this->url_activate[1]) : $itaxuser_mail; //as readed	
		...
		//?? doy can be modified !!!	
        $itaxserial = 'TXC' . strtoupper(hash('crc32',strtoupper(hash('sha1',$iafm . $idoy)))); //tax machine serial number
		$itaxactive = $itaxactive ? $itaxactive : '0';
		...
		$itaxheader = /*$itaxheader ? $itaxheader :*/ 
		              stripslashes(str_replace("'","",$ionomasia.'-'.$icommerTitle."<br>".$iactLongDescr."<br>".
		...
		if ($filtername = $_POST['filtername']) {
		    ...
		    $itaxactive = $_POST['itaxactive'] ? stripslashes($_POST['itaxactive']) : '0';
...
EOF;
        ...
        //$nobuttons = is_array($this->url_activate) ? true : false; //no buttons while in activation mode
...
EOF;
	...
	protected function config_filter_select_form($filter=null, $printername=null, $code=null, $indir=null) {	
		...
		if ($filter=='taxcalc') {
		    //$form = 'dropbox';
		    $form = self::config_filter_form_taxcalc($filter, $printername, $code, $indir);
	    }
	    else
	...
	public function form_infoprinter($printername=null, $indir=null) {
		...
		return (self::html_window(null, $ret, 'taxcloud.printer'));	
    }	
	...
	protected function html_get_printer_jobs_info() {
		...
		$ret .= self::printline(array('No','Date','Size','Name','Status','View'),
		...
		if (is_array($jobs)) {
			...
			foreach ($jobs as $jid=>$fileattr) {			   
			   ...
			   if ($job_status!='completed') { 
			     ...
			     //view file...			 
                 $view = "<a href='$this->urlpath?".$this->cmd."show&iframe=1&job=".$job_id."'>View</a>";  				
			   }	 
		   
               $ret .= self::printline(array($i++,$job_time,$job_size,$job_name,$proceed_state,$view),
			                           array('left;5%','left;30%','left;10%','left;45%','left;5%','left;5%'),
                                       0,
			                           "center::100%::0::group_article_body::left::0::0::");									   									   
			}
...
EOF;
	...
	//override for txt, iso-8859-7 header
		    ...
		    if (substr($fileread,0,4)=='job'.FILE_DELIMITER) {
				...
				if ($jid==$job_id) {
				    ...
				    if ($iframe) {
					  ...
					  $out = self::html_job_viewer($fileread, null, '.txt', 'iso-8859-7'); //<<<<<<
					  die($out);//iframe
					}
